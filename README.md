...
自动执行 `npm install` + `npm run build`，产物输出到 `dist/`。

### 一键部署

在目标服务器上执行（需要 root 权限）：

```bash
curl -fsSL https://raw.githubusercontent.com/yhyyz/aos-sizing-tool/main/deploy.sh | sudo bash
```
...
脚本自动完成：
1. 安装 uv 和 Node.js（如未安装）
2. 从 GitHub clone 代码到 `/opt/app/aos/aos-sizing-tool`
3. 创建 Python 3.12 虚拟环境并安装依赖
4. 构建前端
5. 创建 systemd 服务 `aos-api`（`DATA_SOURCE=api`）
6. 启动服务，监听 9989 端口

### 服务管理

```bash
systemctl status aos-api
systemctl restart aos-api
journalctl -u aos-api -f
```
